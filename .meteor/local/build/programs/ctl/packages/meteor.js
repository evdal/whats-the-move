  ...
  get: function () {                                                                                // 20
    ...
    return Fiber.current._meteor_dynamics &&                                                        // 23
      Fiber.current._meteor_dynamics[this.slot];                                                    // 24
                                                                                                    ...
                                                                                                    // 26
  withValue: function (value, func) {                                                               // 27
    Meteor._nodeCodeMustBeInFiber();                                                                // 28
                                                                                                    // 29
    if (!Fiber.current._meteor_dynamics)                                                            // 30
      Fiber.current._meteor_dynamics = [];                                                          // 31
    var currentValues = Fiber.current._meteor_dynamics;                                             // 32
                                                                                                    // 33
    var saved = currentValues[this.slot];                                                           // 34
    try {                                                                                           // 35
      currentValues[this.slot] = value;                                                             // 36
      var ret = func();                                                                             // 37
    } finally {                                                                                     // 38
      currentValues[this.slot] = saved;                                                             // 39
    }                                                                                               // 40
                                                                                                    // 41
    return ret;                                                                                     // 42
  }                                                                                                 // 43
                                                                                                    ...
                                                                                                    // 45
// Meteor application code is always supposed to be run inside a                                    // 46
// fiber. bindEnvironment ensures that the function it wraps is run from                            // 47
// inside a fiber and ensures it sees the values of Meteor environment                              // 48
// variables that are set at the time bindEnvironment is called.                                    // 49
//                                                                                                  // 50
// If an environment-bound function is called from outside a fiber (eg, from                        // 51
// an asynchronous callback from a non-Meteor library such as MongoDB), it'll                       // 52
// kick off a new fiber to execute the function, and returns undefined as soon                      // 53
// as that fiber returns or yields (and func's return value is ignored).                            // 54
//                                                                                                  // 55
// If it's called inside a fiber, it works normally (the                                            // 56
// return value of the function will be passed through, and no new                                  // 57
// fiber will be created.)                                                                          // 58
//                                                                                                  // 59
// `onException` should be a function or a string.  When it is a                                    // 60
// function, it is called as a callback when the bound function raises                              // 61
// an exception.  If it is a string, it should be a description of the                              // 62
// callback, and when an exception is raised a debug message will be                                // 63
// printed with the description.                                                                    // 64
Meteor.bindEnvironment = function (func, onException, _this) {                                      // 65
  Meteor._nodeCodeMustBeInFiber();                                                                  // 66
                                                                                                    // 67
  var boundValues = _.clone(Fiber.current._meteor_dynamics || []);                                  // 68
                                                                                                    // 69
  if (!onException || typeof(onException) === 'string') {                                           // 70
    var description = onException || "callback of async function";                                  // 71
    onException = function (error) {                                                                // 72
      Meteor._debug(                                                                                // 73
        "Exception in " + description + ":",                                                        // 74
        error && error.stack || error                                                               // 75
      );                                                                                            // 76
    };                                                                                              // 77
  }                                                                                                 // 78
                                                                                                    // 79
  return function (/* arguments */) {                                                               // 80
    var args = _.toArray(arguments);                                                                // 81
                                                                                                    // 82
    var runWithEnvironment = function () {                                                          // 83
      var savedValues = Fiber.current._meteor_dynamics;                                             // 84
      try {                                                                                         // 85
        // Need to clone boundValues in case two fibers invoke this                                 // 86
        // function at the same time                                                                // 87
        Fiber.current._meteor_dynamics = _.clone(boundValues);                                      // 88
        var ret = func.apply(_this, args);                                                          // 89
      } catch (e) {                                                                                 // 90
        // note: callback-hook currently relies on the fact that if onException                     // 91
        // throws and you were originally calling the wrapped callback from                         // 92
        // within a Fiber, the wrapped call throws.                                                 // 93
        onException(e);                                                                             // 94
      } finally {                                                                                   // 95
        Fiber.current._meteor_dynamics = savedValues;                                               // 96
      }                                                                                             // 97
      return ret;                                                                                   // 98
    };                                                                                              // 99
                                                                                                    // 100
    if (Fiber.current)                                                                              // 101
      return runWithEnvironment();                                                                  // 102
    Fiber(runWithEnvironment).run();                                                                // 103
  };                                                                                                // 104
};                                                                                                  // 105
                                                                                                    // 106
//////////////////////////////////////////////////////////////////////////////////////////////////////
